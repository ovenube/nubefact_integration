...
from utils import tipo_de_comprobante, get_serie_correlativo, get_moneda, get_igv, get_tipo_producto
import requests
import json

@frappe.whitelist()
def send_document(invoice):
    doc = frappe.get_doc("Sales Invoice", invoice)
    url = get_url()
    headers = get_autentication()
    serie, correlativo = get_serie_correlativo(doc.name, doc.contingencia)
    igv, monto_impuesto = get_igv(doc.name)
    content = {
            ...
            "porcentaje_de_igv": str(igv),
            "descuento_global": "",
            "total_descuento": "",
            "total_anticipo": "",
            "total_gravada": str(round(doc.net_total, 2)),
            "total_inafecta": "",
            "total_exonerada": "",
            "total_igv": str(round(monto_impuesto, 2)),
            "total_gratuita": "",
            "total_otros_cargos": "",
    ...
    for item in doc.items:
        ...
        content['items'].append({
            ...
            "subtotal": str(round(item.base_net_amount, 2)),
            "tipo_de_igv": "1",
            "igv": str(round(item.amount - item.net_amount, 2)),
