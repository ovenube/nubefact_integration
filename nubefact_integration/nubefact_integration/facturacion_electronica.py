                ...
                if doc.sales_invoice_advance is not None:
                    ...
                    for item in doc.items:
                        ...
                        content['items'].append({
                            ...
                            "valor_unitario": str(round(item.unit_value, 2)) if (doc.total_amount_free != 0) else str(round(item.net_rate, 2)),
                            "precio_unitario": str(round(item.unit_value, 2)) if (doc.total_amount_free != 0) else str(round(item.rate, 2)) if igv_inc == 1 else str(round(item.net_rate, 2) * 1.18),
